...
impl Renderer {
    ...
    pub fn render(&self, mut output_file: &File) -> anyhow::Result<()> {
        ...
        write!(
            output_file,
            r#"# rules_rust_external file format 1
# config hash {hash}
...
"#,
                    ...
                    repository_name =
                        config::crate_to_repo_rule_name(&package.pkg_name, &package_version),
...
"##,
            crate_repo_names_inner = self
                ...
                .map(|(crate_name, crate_version)| format!(
                    r#"    "{}": "{}",{newline}"#,
                    crate_name,
                    config::crate_to_label(&crate_name, &crate_version.to_string()),
                    newline = '\n',
                ))
                .collect::<Vec<String>>()
                .join(""),
