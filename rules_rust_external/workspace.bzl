...
def _crate_universe_resolve_impl(repository_ctx):
    ...
    Environment Variables:
        RULES_RUST_EXTERNAL_UPDATE_LOCKFILE: Re-pin the lockfile if `true`.
        RULES_RUST_RESOLVER_URL_OVERRIDE: Override URL to use to download resolver binary - for local paths use a file:// URL.
    """
  ...
  "cargo": "{cargo}"
    ...
    #   export RULES_RUST_RESOLVER_URL_OVERRIDE=file:///path/to/rules_rust_external/resolver/target/release/resolver
    resolver_url = repository_ctx.os.environ.get("RULES_RUST_RESOLVER_URL_OVERRIDE", None)
    ...
    if repository_ctx.os.environ.get("RULES_RUST_EXTERNAL_UPDATE_LOCKFILE", "false") == "true":
...
Example:
    ...
    load("@rules_rust_external//:workspace.bzl", "rust_library", "crate")
...
"""),
        ...
        "_resolver_script_linux": attr.label(
            default = "@rules_rust_external_resolver_linux//file:downloaded",
            executable = True,
            cfg = "host",
        ),
        "_resolver_script_darwin": attr.label(
            default = "@rules_rust_external_resolver_darwin//file:downloaded",
            executable = True,
            cfg = "host",
        ),
    },
